...
public class S1BasicScript : MonoBehaviour
{
    ...
    {
        if ((string)temp[2] == "S" && this.transform.position.x == 0.0f)
        {
            if (PlayerNum == 2)
            {
            ...
            MundD = new Vector3(PosData.XAxis[(int)temp[0]] + 0.5f, PosData.YAxis[(int)temp[1]], this.transform.position.z);
